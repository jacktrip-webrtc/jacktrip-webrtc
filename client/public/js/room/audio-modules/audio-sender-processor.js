...
class AudioCircularBuffer {
    ...
    enqueue(packet, envelop=null, isInputInt16=true, hasPacketHeader=true, ...args) {
        ...
        // for testing only
        if(this.log) console.log('packet.packet_n', packet.packet_n, '_latestPacket_nDequeued', _latestPacket_nDequeued);

        // safety check
        if(packet.packet_n <= _latestPacket_nDequeued) {
            if(this.log || this.config.logDiscardedPackets) console.error(`AudioCircularBuffer.enqueue: packet too late for this index, discarded ${packet.packet_n}/${_latestPacket_nDequeued}`);
            if(this.balancedOffset) {
...
class AudioSenderProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.isLocalLoopback = false; // signal if it is in loopback state
        ...
        this.useWaitAsync = false; // working w/ Chrome only!
        this.waitAsyncNotificationSharedBuffer = null;
        this.rxQueue = null; // receiving queue, for local audio loopback
        this.txQueue = null; // transmission queue
        this.config = null

        this.useTestSamples = false;
        this.packetManager = new PacketManager(this.log, this.useTestSamples);
        this.isSenderMutedSharedBuffer = null;

        this.isSinActive = false;
        this.sin = null;
        this.sinIndex = 0;
        this.sinLength = 0;
        ...
        this.port.onmessage = (event) => {
            ...
            if(this.log) console.log(`AudioSenderProcessor.onmessage, ${data.type}`);
            ...
            switch(data.type) {
                ...
                case 'config':
                    ...
                    this.packetManager = new PacketManager(this.log, 0n, this.useTestSamples);

                    // this is an audio source
                    if(this.isLocalLoopback && data.rxCircularBuffer)
                        this.rxQueue = new AudioCircularBuffer(this.config, data.rxCircularBuffer, true, this.log, true);
                    if(this.useWaitAsync && data.txCircularBuffer)
                        this.txQueue = new AudioCircularBuffer(this.config, data.txCircularBuffer, true, this.log, false);

                    if(this.config.addSin) this._generateSin();

                    if(this.log) console.log(`AudioSenderSenderProcessor.onmessage, is local loopback = ${this.isLocalLoopback}`);
                ...
                case 'track-status':
                    ...
                    break;
                case 'destroy':
                    this.rxQueue?.destroy();
                    this.txQueue?.destroy();
                    this.isTerminated = true;
                    break;
                case 'loopback':
                    this.isLocalLoopback = data.isLocalLoopback;
                    break;
                case 'sin':
                    this.isSinActive = data.isSinActive;
                    break;
                case 'log':
                    this.log = data.log;
                    break;
                case 'channelList':
                    let channels = data.channelList.split(',');
                    this.source = {
     ...
     */
    _generateSin() {
        let nSamples = this.config.audioContextOptions.sampleRate / this.config.sinFreq;
        let sin = [];
        let step = 2 * Math.PI / nSamples;

        for(let s=0, x=0; s<nSamples; s++, x+=step)
            sin.push(Math.sin(x));

        // the processor is already running
        this.sin = sin;
        if(this.log) console.log(`AudioSenderSenderProcessor.onmessage, sin generated at freq ${this.config.sinFreq} Hz!`);
    }
    ...
    process(inputs, outputs, parameters) {
        ...
        let _isMuted = true;
        if(this.config)
            _isMuted = this.config.useSharedBufferForProcessorStatus ? Atomics.load(this.isSenderMutedSharedBuffer, 0) : this.isMuted;
        ...
        if(!_isMuted) {
            ...
            //     }
            // }
            // if(silent && this.log) console.error('INPUT IS SILENT');
            if(input.some((el) => !el)) console.error('WRONG INPUT');

            if(this.isSinActive)
                // add sinusoid to detect missing packets on the receiver
                for(let c=0; c<input.length; c++) { // for each channel
                    for(let s=0; s<this.config.windowSize; s++) { // for each sample
                        input[c][s] = input[c][s] * (1-this.config.sinGain)
                            + this.sin[this.sinIndex] * this.config.sinGain;
                        this.sinIndex = this.sinIndex == this.sin.length-1 ? 0 : this.sinIndex+1;
                    }
                }

            // if(!silent) {
                if(this.useWaitAsync) { // waitAsync 
                    let _input = { buffer: input };

                    // enqueue w/out create the packet structure
                    if(this.isLocalLoopback) { // if loopback, enqueue in receiving queue
                        if(this.log && !this.rxQueue) // safety check
                            console.error(`AudioSenderProcessor, no receiving queue`);
                        else
                            this.rxQueue.enqueue(_input, null, false, false); // direct insert into receiving queue. Enqueue Float32 samples, no header
                    }
                    else { // standard case, enqueue in trasmission queue
                        if(this.log && !this.txQueue) // safety check
                            console.error(`AudioSenderProcessor, no trasmission queue`);
                        else {
                            this.txQueue.enqueue(_input, null, false, false); // enqueue Float32 samples, no header
                            Atomics.notify(this.waitAsyncNotificationSharedBuffer, 0, 1); // notify the main thread
                            if(this.log) console.log(`AudioSenderProcessor, enqueued and notified for waitAsync`);
                        }
                    }
                }
                else { // postMessage, standard case
                    // create the buffer structure
                    let buffer = this.packetManager.create({
                        source: this.source, // channel info
                        samples: input, // all samples
                    });
                    ...
                    if(this.isLocalLoopback) { // if loopback, enqueue in receiving queue
                        if(this.log && !this.rxQueue) // safety check
                            ...
                            this.rxQueue.enqueue(packet, null, true, true);
                        }
                    }
                    else // standard case, send to main thread
                        this.port.postMessage(buffer); // simplified version than Matteo's architecture: send only buffer
                }
            // }
        }

        // // For testing only
        // for(let c=0; c<outputs.length; c++) // For each channel
        //     for(let s=0; s<outputs[c].length; s++) // For each sample
        //         outputs[c][s] = 0.;
