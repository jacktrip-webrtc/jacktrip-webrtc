...
class AudioCircularBuffer {
    ...
    enqueue(packet, envelop=null, isInputInt16=true, hasPacketHeader=true, ...args) {
        ...
        if(Math.abs(Number(packet.packet_n - _latestPacket_nDequeued)) >= this.queueSize) {
            if(this.log) console.error(`AudioCircularBuffer.enqueue: packet too late or too early for this index, discarded ${packet.packet_n}/${_latestPacket_nDequeued}`);
...
/**
 * single sender and receiver processor
 * with adaptations to new Paolo's architecture
 * 
 * @author Sacchetto Matteo
 * @author Gastaldi Paolo
 */
class AudioSenderReceiverProcessor extends AudioWorkletProcessor {
    constructor() {
        super();
        ...
        this.port.onmessage = (event) => {
            ...
            if(this.log) console.log(`AudioSenderReceiverProcessor.onmessage, ${data.type}`);
            ...
            switch(data.type) {
                ...
                case 'config':
                    ...
                    if(this.log) console.log(`AudioSenderReceiverProcessor.onmessage, using message channel = ${this.useMessageChannel}`);
                    break;
     ...
     */
    _generateSin() {
        let nSamples = this.config.sampleRate / this.config.sinFreq;
        let sin = [];
...
registerProcessor('audio-sender-receiver-processor', AudioSenderReceiverProcessor);
